...
/** Project-wide locals generated from `defineConfig({ locals })`. */
export interface RuneConfigLocals {}
...
export type InferConfigLocals<TConfig, TFallback = RuneConfigLocals> = TConfig extends {
  readonly locals?: infer TLocals;
}
  ? NonNullable<TLocals> extends (...args: any[]) => infer TResult
    ? Awaited<TResult>
    : TFallback
  : TFallback;
...
export interface CommandContext<TOptions, TArgs, TLocals = RuneConfigLocals> {
  ...
  /** Project-defined runtime values generated by `defineConfig({ locals })`. */

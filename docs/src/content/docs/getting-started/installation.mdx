...
1. **Create a new project**
   ...
   To scaffold into the current directory, pass `.` instead of a project name. The directory must not contain files that conflict with the template (e.g. `package.json`, `src/`).
...
Pass the `--yes` (`-y`) flag to skip all interactive prompts and use default values. This also happens automatically in non-TTY environments such as CI pipelines or agent-driven workflows.

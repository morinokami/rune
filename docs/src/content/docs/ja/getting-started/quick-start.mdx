...
✓ tests/commands/hello.test.ts (1 test) 1ms
✓ tests/commands/text/count.test.ts (2 tests) 2ms
...
上で述べたように、スターターにはトップレベルの `hello` だけでなく、`text` というコマンドグループも含まれています。`src/commands/text/` のように `src/commands` 以下にディレクトリを作ると、その名前が複数のコマンドを束ねるコマンドグループとなり、中にあるファイルがサブコマンドとして公開されます。
...
<Tabs syncKey="pkg">
  <TabItem label="pnpm">
  ```sh
  $ pnpm run start text count "hello rune world"
  3
  ```
  </TabItem>
  <TabItem label="npm">
  ...
  <TabItem label="yarn">
  ```sh
  $ yarn run start text count "hello rune world"
  3
  ```
  </TabItem>
  <TabItem label="bun">
  ```sh
  $ bun run start text count "hello rune world"
  3
  ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="pkg">
  <TabItem label="pnpm">
  ```sh
  $ pnpm run start text transform hello
  HELLO
  ```
  </TabItem>
  <TabItem label="npm">
  ...
  <TabItem label="yarn">
  ```sh
  $ yarn run start text transform hello
  HELLO
  ```
  </TabItem>
  <TabItem label="bun">
  ```sh
  $ bun run start text transform hello
  HELLO
  ```
  </TabItem>
</Tabs>
...
<Tabs syncKey="pkg">
  <TabItem label="pnpm">
  ```sh
  $ pnpm run start text transform HELLO --mode lower
  hello
  ```
  </TabItem>
  <TabItem label="npm">
  ...
  <TabItem label="yarn">
  ```sh
  $ yarn run start text transform HELLO --mode lower
  hello
  ```
  </TabItem>
  <TabItem label="bun">
  ```sh
  $ bun run start text transform HELLO --mode lower
  hello
  ```
  </TabItem>
</Tabs>
...
```
Usage: my-cli text transform <input> [options]
...
hello コマンドのテストと同様に、`transform` コマンドのテストも書いてみましょう。`tests/commands/text/transform.test.ts` を作成します:

```ts
// tests/commands/text/transform.test.ts
import { runCommand } from "@rune-cli/rune/test";
import { expect, test } from "vitest";

import transformCommand from "../../../src/commands/text/transform";
...
✓ tests/commands/hello.test.ts (1 test) 1ms
✓ tests/commands/text/count.test.ts (2 tests) 2ms
✓ tests/commands/text/transform.test.ts (2 tests) 2ms

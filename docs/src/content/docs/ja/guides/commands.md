...
以下は英語で挨拶するコマンドの定義例です。まず `options` でオプション、`args` で位置引数を定義し、その後に `run` 関数でコマンドの処理を記述しています:
...
`index.ts`、`_group.ts` のような予約メタデータファイル、`_` で始まるファイル、テストファイルを除く `.ts` ファイルは、そのファイル名がそのままサブコマンド名になります。たとえば `src/commands/project/create.ts` は `your-cli project create` に対応します。

ディレクトリを作らずにサブコマンドを定義できるため、子コマンドが不要なシンプルなコマンドに向いています。

### コロケーション

コマンド名が `_` で始まるファイルやディレクトリはルーティング対象外です。コマンド固有の実装詳細を、そのコマンドの近くに置きたい場合に使えます:
...
`.test.ts` または `.spec.ts` で終わるテストファイルもルーティング対象外です:
...
上記の例では、コマンドとして認識されるのは `deploy` と `project create` のみとなります。
...
`_group.ts` はコマンドグループのメタデータを定義するための予約ファイルです。予約ファイル名が `_group.ts` であるため、`group.ts` は通常のコマンドファイルとして使用できます。
...
説明文がある場合のデフォルトのヘルプ出力はこのかたちとなり、説明文は `Usage:` の前に表示されます。

## enum フィールド

値を固定された選択肢のいずれかに制限したい場合は、`type: "enum"` と `values` のリストを使用します。文字列と数値を値として指定でき、許可された値の共用体型は自動的に推論されます。選択肢は `--help` にも表示されます。
...
オプションが複数回指定可能な場合、`--tag alpha --tag beta` のような指定や、長い形式と短い形式の混在が受け付けられます。`multiple: true` を設定していないオプションを複数回指定した場合はエラーになります。

複数回指定可能なオプションは、プリミティブの文字列/数値オプション、enum オプション、スキーマ値オプションで使用できます。プリミティブの真偽値オプションと、スキーマの `flag: true` オプションでは使用できません。スキーマ値オプションでは、収集された生の文字列値が配列としてスキーマに渡されるため、`z.array(z.string()).default([])` のような配列スキーマを使用してください。

## 環境変数によるフォールバック

単一値のオプションには `env` フォールバックを宣言できます。Rune は、そのオプションが CLI で指定されなかった場合にのみ環境変数を読みます:
...
優先順位は **CLI > env > default** です。`--port 4000` は `PORT=5000` より優先されます。`--port` が省略された場合は `PORT=5000` がオプション値として解析され、どちらも存在しない場合はデフォルト値が使われます。

環境変数の値は CLI 値と同じ解析・バリデーション規則で扱われます。不正な環境変数の値はデフォルト値にフォールバックせず、コマンドをエラーにします。`env` は型推論には影響せず、複数回指定可能なオプションでは使用できません。
...
プリミティブの真偽値オプションに `default: true` を設定すると、Rune は `--no-<name>` フラグを自動生成し、デフォルト値を上書きできるようにします:
